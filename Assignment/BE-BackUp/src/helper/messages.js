import { faC } from "@fortawesome/free-solid-svg-icons";

const MESSAGES = {
    OTPSent: "Otp has been sent to mail",
    Include_users_who_have_been_disabled: 'Include users who have been disabled',
    LOGIN_FAILED: "Login Failed",
    Roles_Select_Role_As_Agreed_By_Headteacher: 'Roles - Select Role(s) as agreed by Headteacher',
    Classes_Select_Classes_That_This_User_Will_Be_Teaching_Or_Supporting: 'Classes - Select Classes that this user will be teaching or supporting',
    This_Combination_Already_Exists: 'This combination Already Exists',
};
const VALIDATION = {
    YOUR_ROLE_IS_REQUIRED: "*Your Role is required",
    NAME_IS_REQUIRED: "Name is required",
    NAME_MUST_HAVE_AT_LEAST_2_CHARACTERS: "*Names must have at least 2 characters",
    NAME_CANT_BE_LONGER_THAN_50_CHARACTERS: "*Name can't be longer than 50 characters",
    NAME_CANT_BE_LONGER_THAN_100_CHARACTERS: "*Name can't be longer than 100 characters",
    SELECT_ANY_ONE: "Select any one",
    MUST_BE_A_VALID_EMAIL_ADDRESS: "Must be a valid email address",
    EMAIL_IS_REQUIRED: "Email is required",
    OTP_IS_REQUIRED: "*OTP is required",
    OTP_MUST_HAVE_6_CHARACTERS: "*Otp must have 6 digits",
    OTP_MUST_HAVE_6_DIGITS: "*Otp must contain only 6 digits",
    PASSWORD_MUST_HAVE_MINIMUM_8_CHARACTERS: "*Password must have minimum 8 characters",
    PASSWORD_MUST_HAVE_AT_LEAST_ONE_UPPERCASE_LOWERCASE_NUMBER_SPECAIL_CHARACTER: "*Password must have at least one uppercase letter, one lowercase letter, one number and one special character",
    OLD_PASSWORD_IS_REQUIRED: "Old password is required",
    PLEASE_CONFIRM_THE_PASSWORD: "Please confirm the password",
    NUMBERS_ARE_NOT_ALLOWED_FOR_USERNAME: "Username should contain only alphabets!",
    PASSWORD_MUST_MATCH: "Passwords must match",
    EMAIL_MUST_MATCH: "Email must match",
    SUCCESSFULLY_APPLIED: "Successfully Applied",
    SUCCESSFULLY_REVERTED: "Successfully Reverted",
    SUCCESSFULLY_UPDATED: "Successfully Updated",
    EMPTY_SPACE_NOT_ALLOWED: "Empty spaces not allowed!",
    FIRST_NAME_IS_REQUIRED: "Full name is required!",
    LAST_NAME_IS_REQUIRED: "Nick name is required!",
    NAME_SHOULD_CONTAIN_ATLEAST_3_CHARACTERS: "Names should contain atleast 2 characters",
    PHONE_NUMBER_CANNOT_BE_EMPTY: "Phone number cannot be empty!",
    IS_POSITIVE: "Is positive?",
    COMMENT_IS_REQUIRED: "comment is required!",
    PLEASE_SELECT_A_SUBJECT: 'Please Select Subject',
    PLEASE_SELECT_A_CLASS: 'Please Select Class',
    PLEASE_SELECT_A_ROLE: 'Please Select Role',
    USER_IS_ALREADY_DISABLED_DO_YOU_WANT_TO_DELETE_THE_ROLE: 'User is Already Disabled Do you want to delete the Role',
    USER_IS_ALREADY_DISABLED_DO_YOU_WANT_TO_DELETE_THE_CLASS: 'User is Already Disabled Do you want to delete the Class',
    MarksUpdated: "marks successfully updated!",
}
const ERROR = {
    FETCHING_DATA: "Sorry, there was a problem trying to fetch data.",
    UPDATE: "Sorry, there was a problem trying to perform the update. Your changes are not saved.",
}

const SUCCESS = {
    User_Invited: 'Successfully user is invited'
}

const termsConditions = {
    tc: 'I have read and accept the terms and conditions for use of Mappix.',
}

export { MESSAGES, VALIDATION, ERROR, SUCCESS, termsConditions };